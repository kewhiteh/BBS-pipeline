...
def test_synthetic_zip_fixture_in_memory(synthetic_zip_builder, sample_routes_csv_content):
...
def test_corrupted_stream_failure_assertion(corrupted_byte_stream):
    ...
    with pytest.raises(zipfile.BadZipFile):
        with zipfile.ZipFile(corrupted_byte_stream) as zf:
            zf.namelist()
...
def test_empty_stream_failure_assertion(empty_byte_stream):
    ...
    with pytest.raises(zipfile.BadZipFile):
        with zipfile.ZipFile(empty_byte_stream) as zf:
            zf.namelist()
