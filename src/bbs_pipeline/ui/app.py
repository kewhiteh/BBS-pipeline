...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.divider()
        st.header("📍 Spatial Processing")
        spatial_enabled = st.checkbox("Enable 50-Stop Spatial Processing", value=False)

        spatial_filter_mode = "STRICT_CONTAINMENT"
        spatial_filter_path = None
        route_shapefile_path = None

        if spatial_enabled:
            st.info("Spatial filtering activated.")

            spatial_filter_mode = st.selectbox(
                "Filter Mode",
                options=[
                    "STRICT_CONTAINMENT",
                    "ANY_STOP_INTERSECT",
            ...
            poly_uploader = st.file_uploader(
                "Upload Spatial Filter (GeoJSON Polygon)", type=["geojson", "json"]
            )
            if poly_uploader is not None:
                try:
                    content = poly_uploader.read().decode("utf-8")
                    json.loads(content)
                    with tempfile.NamedTemporaryFile(
                        mode="w", delete=False, suffix=".geojson"
                    ) as tmp:
                        tmp.write(content)
                        spatial_filter_path = tmp.name
                except Exception as e:  # noqa: BLE001
                    st.error(f"Invalid GeoJSON: {e}")

            route_uploader = st.file_uploader(
                "Upload Route Polyline (GeoJSON LineString)", type=["geojson", "json"]
            )
            if route_uploader is not None:
                content = route_uploader.read().decode("utf-8")
                with tempfile.NamedTemporaryFile(
                    mode="w", delete=False, suffix=".geojson"
                ) as tmp:
                    tmp.write(content)
                    route_shapefile_path = tmp.name
        st.header("📦 Output Configuration")
        export_format = st.selectbox(
            "Serialization Format",
            options=["parquet", "csv", "gpkg", "fgb", "geojson"],
            index=0,
    ...
    if not valid_coords_df.is_empty():
        ...
        if spatial_enabled:
            st.info(
                "Spatial mode active. Run pipeline to export vector layers containing 50-stops."
            )
            st.pydeck_chart(
                pdk.Deck(
                    map_style=None,
                    initial_view_state=pdk.ViewState(
                        latitude=pdf_coords["Latitude"].mean(),
                        longitude=pdf_coords["Longitude"].mean(),
                        zoom=3,
                        pitch=0,
                    ),
                    layers=[
        ...
        with st.spinner(
            ...
            try:
                ...
                out_bytes = run_pipeline(
                    ...
                    include_stops=spatial_enabled,
                    spatial_filter=spatial_filter_path if spatial_enabled else None,
                    filter_mode=spatial_filter_mode
                    if spatial_enabled
                    else "STRICT_CONTAINMENT",
                    export_format=export_format
                    if export_format in ["gpkg", "fgb", "parquet"]
                    else None,
                    shapefile=route_shapefile_path if spatial_enabled else None,
                )
